...
public class OpenCPSTraceAction extends Action {
	...
	public void run(HttpServletRequest request, HttpServletResponse response)
		...
		if (UserMgtConfigUtil.isTrackUserEnable()) {
			Long userId = request.getAttribute("USER_ID") != null
				? (Long) request.getAttribute("USER_ID") : 0;

			String completeUrl =
				(String) request.getAttribute("CURRENT_COMPLETE_URL");

			String sessionId = request.getSession() != null
				? request.getSession().getId() : StringPool.BLANK;
			try {
				UserLogin userLogin =
					UserLoginLocalServiceUtil.fetchByU_S(userId, sessionId);
				if (userLogin != null) {
					...
					UserTrackPathLocalServiceUtil.updateUserTrackPath(
						userLogin.getCompanyId(), 0, userLogin.getUserLoginId(),
						completeUrl, new Date());
				}
			}
